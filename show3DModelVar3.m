function show3DModelVar3(DisMtrx1,DisMtrx2,DisMtrx3,out,re1,re2,re3)
% this function is used for 3D case
...
% input: DisMtrx3 is the distance matrix for SNESIM realizations
% input: out is the training image e.g. 69*69*39
% input: re1 is DISPAT realizations: e.g. 69*69*39*50
% input: re2 is CCSIM realizations
% input: re3 is SNESIM realizations
...
for ii=1:5
    ...
    scatter3(x_sisim,y_sisim,z_sisim,S,'g','filled');
    hold on;
    
    scatter3(0,0,0,100,'k','filled');
    legend('Training image','DISPAT','CCSIM','SNESIM');
...
%     text(x_CCSIM(IX2(ii,51)),y_CCSIM(IX2(ii,51)),'\rightarrow 50','FontSize',10,'Color','b');
    ...
    scatter(x_sisim,y_sisim,S,'g','filled');
...
%     text(x_sisim(IX3(ii,51)),y_sisim(IX3(ii,51)),'\leftarrow 50','FontSize',10,'Color','r');
    ...
    legend('Dispat','CCSIM','SNESIM','training image');
...
%     text(x_CCSIM(IX2(ii,51)),z_CCSIM(IX2(ii,51)),'\rightarrow 50','FontSize',10,'Color','b');
 S=20*ones(1,51); S(51)=100;
 S(IX3(ii,2))=30; S(IX3(ii,21))=30; S(IX3(ii,31))=30; S(IX3(ii,41))=30; S(IX3(ii,51))=30;
 scatter(x_sisim,z_sisim,S,'g','filled');
...
xlabel('Dimension');
ylabel('Cumulative');
legend(['Dispat ' num2str(e_dispat_new(3))],['CCSIM ' num2str(e_CCSIM_new(3))],['SNESIM ' num2str(e_CCSIM_new(3))],'Location', 'SouthEast');
title(['Multi Resolution= ' int2str(i)]);
hold off;
